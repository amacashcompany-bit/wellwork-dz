  ...
  return (
    <div className="min-h-screen relative flex items-center justify-center px-4 py-12 gradient-mesh">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="relative w-full max-w-md">
        <Link to="/" className="flex items-center gap-2 justify-center mb-6 text-white">
          ...
          <div>
            ...
            <div className="text-[10px] uppercase tracking-widest text-white/60">QVT · Prévention RPS</div>
          </div>
        </Link>

        <Card className="p-8 rounded-3xl glass-dark border-white/10 text-white shadow-glow">
          <Tabs value={tab} onValueChange={(v) => setTab(v as "signin" | "signup")}>
            ...
            <TabsContent value="signin" className="mt-6 space-y-4">
              <div><Label className="text-white/80">Email</Label>
                <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="mt-1 bg-white/5 border-white/10 text-white placeholder:text-white/40" />
              </div>
            ...
            <TabsContent value="signup" className="mt-6 space-y-4">
              <div><Label className="text-white/80">Nom complet</Label>
                <Input value={fullName} onChange={(e) => setFullName(e.target.value)} className="mt-1 bg-white/5 border-white/10 text-white" />
              </div>
              <div><Label className="text-white/80">Email</Label>
                <Input type="email" value={email} onChange={(e) => setEmail(e.target.value)} className="mt-1 bg-white/5 border-white/10 text-white" />
              </div>
              <div><Label className="text-white/80">Mot de passe</Label>
                <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} className="mt-1 bg-white/5 border-white/10 text-white" />
              </div>
          ...
          <Button onClick={doGoogle} disabled={busy} variant="outline" className="w-full h-11 bg-white text-slate-900 hover:bg-white/90 border-0">
